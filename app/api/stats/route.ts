...
export async function GET() {
  try {
    ...
    const predPath = path.join(cwd, 'nba_historical_predictions.json')
    const gamesPath = path.join(cwd, 'nba_historical_data.json')

    if (fs.existsSync(predPath) && fs.existsSync(gamesPath)) {
      const preds: Array<{ date: string; home_team: string; away_team: string; winner: string }> = JSON.parse(
      ...
      let total = 0
      let correct = 0
      const predictions_history: Array<{ date: string; home_team: string; away_team: string; actual: string; predicted: string; correct: boolean }> = []
    ...
    const filePath = path.join(cwd, 'prediction_stats.json')
